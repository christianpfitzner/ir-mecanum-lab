...
The three numbers, the smallest turning radius, the odometry lesson and the slip figures below are all
measured on the command printed next to them, by `tests/test_steering_w4.py` and by the runs quoted
here — not estimated from the model.

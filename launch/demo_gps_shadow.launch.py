"""Demo: GPS that gets bad by place — the settings of `config/demo_gps_shadow.json`.

    ros2 launch mecanum_lab demo_gps_shadow.launch.py
    ros2 launch launch/demo_gps_shadow.launch.py robot:=alice          # straight from the source tree

What the window should show: fixes that scatter inside the shadow of the high shelf and none at all on the loading dock, while the
odometry ghost (`o`) goes on drawing the honest line the wheels would have produced.

...
def code_lines(path):
    """Lines that carry code — comments and docstrings out.

    The launch files of this repository are allowed to be *explained*: every argument has a line of help
    that `--show-args` prints, and the reason a `sys.path.append` sits where it sits belongs in the file.
    What must not grow there is logic — a launch file is a list of things to start. So the budget counts
    the lines with tokens on them, and prose is not punished.
    """
    import io
    import tokenize
    src = reads(path)
    ...
    for token in tokenize.generate_tokens(io.StringIO(src).readline):
        if token.type in (tokenize.COMMENT, tokenize.NL, tokenize.NEWLINE, tokenize.INDENT,
            ...
            continue
        if token.type == tokenize.STRING and src.split("\n")[token.start[0] - 1].lstrip().startswith(
                ('"""', "'''")):
            continue                                    # a docstring is documentation, not code
...
def test_launch_file_small_and_with_arguments(file_name, args, max_lines):
    ...
    text = reads(path)
    assert code_lines(path) < max_lines, f"{file_name} has {code_lines(path)} code lines, cap {max_lines}"

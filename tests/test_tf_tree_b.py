...
def make_robot(name="alice", pose=(1.0, 2.0, 0.5), odom=(0.9, 2.1, 0.45)):
...
def test_header_frames_match_the_tree(kind, erwartet):
    assert tf_bcast.frame_for(kind, "alice", CFG) == erwartet
...
def test_every_kind_in_the_contract_has_a_frame():
    for kind in ("odom", "scan", "gps", "truth", "imu", "kf"):
        assert tf_bcast.frame_for(kind, "bob", CFG)          # KeyError here means: no frame
        assert tf_bcast.frame_for(kind, "bob", CFG).endswith(tf_bcast.frame_for(kind, "", CFG))
...
def test_static_tree_mounts_laser_and_imu_on_base_link():
    sending = tf_bcast.static_tree(M, engine(make_robot()), CFG, 3.0)
...
def test_static_tree_grows_with_every_robot():
    sending = tf_bcast.static_tree(M, engine(make_robot("alice"), make_robot("bob")), CFG, 1.0)
...
def test_dynamic_tree_publishes_map_and_odometry():
    sending = tf_bcast.dynamic_tree(M, engine(make_robot()), CFG, 5.0)
    paare = {(t.header.frame_id, t.child_frame_id) for t in sending}
...
def test_map_to_odom_is_the_identity_by_default():
    ...
    sending = tf_bcast.dynamic_tree(M, engine(make_robot()), CFG, 1.0)
...
def test_truth_mode_moves_the_odometry_frame():
    ...
    robot = make_robot(pose=(3.0, 1.0, 1.0), odom=(2.0, 0.5, 0.4))
...
def test_two_robots_never_share_a_frame():
    sending = (tf_bcast.static_tree(M, engine(make_robot("alice"), make_robot("bob")), CFG, 1.0)
               + tf_bcast.dynamic_tree(M, engine(make_robot("alice"), make_robot("bob")), CFG, 1.0))
    kinder = [t.child_frame_id for t in sending]
